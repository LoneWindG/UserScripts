...
// @match        https://switch-software-compatibility.nintendo.com/*
...
function getBuiltinRegionMap() {
    return new Map([
        ...
        ["ns_regions", [ "[zh-HK]香港", "[ja-JP]日本", "[en-US]美国" ]],
    ]);
}
...
function verifyDomain() {
    ...
    else if (domain.endsWith("switch-software-compatibility.nintendo.com")) {
        regionKey = "ns_regions";
        changeMethod = changeNsWebUrl;
    }
    else {
...
function registerRegions(url, regionKey, changeMethod, showAllCodes = false)
{
    ...
    for(var region of regions)
    {
        ...
        var code = region.substring(codeStart + 1, codeEnd);
        if (code.toLowerCase() === currentCode) {
            continue;
        }
...
function getCurrentCode(url)
{
    ...
    // console.log("[网站地区&语言快捷切换工具] 当前PathName: " + pathname);
    var codeEnd = pathname.indexOf('/', 1);
    if (codeEnd === -1) {
        if (pathname.length === 6 && pathname[3] === '-') { // pathname like: /zh-CN
...
function changeRegion(code)
{
    ...
    var newHerf = "";
    if (href.endsWith(`/${currentCode}`)){
        newHerf = href.replace(`/${currentCode}`, `/${code}`);
    }
    else {
        newHerf = href.replace(`/${currentCode}/`, `/${code}/`);
    }
    if (newHerf === href)
    {
        alert(`地区切换失败, 替换当前地区代码[${currentCode}]为目标地区代码[${code}]后网址未发生变化`);
        return;
    }
...
function changeNsWebUrl(code) {
    document.cookie = `NEXT_LOCALE=${code}`;
    changeRegion(code);
}
